...
def readSTU(filename):
    ...
    for straghgikh in strategikes:                           # Δοκιμή και των τριών στρατηγικών για το πρόβλημα
        ...
        maxXrwmata = 0
        #βρήσκεται ο αριθμός των χρωμάτων (περιόδων) που χρησιμοποιήθηκε
        for mathima in xrwmatismos:
            if xrwmatismos[mathima] > maxXrwmata:
                maxXrwmata = xrwmatismos[mathima]
        #Εάν έχουν χρησιμοποιηθεί λιγότερες περίοδοι κρατήτε αυτήν την στρατηγική και τον χρωματισμό
    ...
    bathmologia = 0
    # Ελέγχω όλους τους συνδιασμούς μαθημάτων πχ (1,2), (1,3) ... (2,3) από μία φορά
    for i in range(1, megalyteroMathima + 1):
        for j in range(i + 1, megalyteroMathima + 1):
            ...
            if key in koinoispoudastes.keys():
                ...
                diafora = abs(d[i] - d[j])
                # Βρίσκέται με βάσει το dictionary τους κοινούς σπουδαστές
